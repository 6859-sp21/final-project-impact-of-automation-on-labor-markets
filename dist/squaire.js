...
(function (root, factory) {
	...
	return function () {
		...
		tip.style = function (n, v) {
			if (arguments.length < 2 && typeof n === 'string') {
				...
				var args = Array.prototype.slice.call(arguments)
				d3v3.selection.prototype.style.apply(getNodeEl(), args)
			}
		...
		tip.toolbox = function (v) {
			...
			toolbox = v == null ? v : d3v3.functor(d3_tip_toolbox(v))
		...
		function d3_tip_toolbox(el) { return d3v3.select(el) }
	...
	function Squaire(data, options) {
		...
		var defaults = {
			...
			colors: d3v3.scale.ordinal().range(['#f3f3f3']),//d3 color scale
		...
		this.$el = d3v3.select(this.options.el)
			.append("div")
		...
		if (this.options.tooltip.enabled) {
			...
			this.toolbox = d3v3.select(self.options.tooltip.el).attr("class", "squaire-toolbox");
		...
		d3v3.select(window).on('resize.' + this.options.el, resize.bind(this));
	...
	*/
	...
	Squaire.prototype.mapDimensions = function () {
		var boxesWide = d3v3.extent(this.data, function (d) { return d.x; });
		var boxesTall = d3v3.extent(this.data, function (d) { return d.y; });
		...
		if (this.options.tooltip.enabled) {
			...
			this.boxRect
				.on('mouseover', function (d, idx) {
					...
					d3v3.selectAll("g.active").classed("active", false);
					var box = d3v3.select(this.parentNode).classed("active", true).node();
